...
We denote this shortest path weight as the distance $\operatorname{dist}_w(s,t)$ between $s$ and $t$.

\subsection{Dijkstra's Algorithm}

Dijkstra's algorithm can be used to solve this point to point shortest path problem.
...
The priority queue is then ordered by $d[v] + h_t(v)$.
Thus, nodes closer to the target a explored earlier.
See Figure~\ref{img:search-space} for an example of nodes traversed during an A* search.
A*'s running time crucially depends on how tight this estimate is.

A* is equivalent to running Dijkstra's algorithm on the graph with \emph{reduced weights} $w_{h_t}$.
This reduced weight function is defined as $w_{h_t}(uv) := w(uv) - h_t(u) + h_t(v)$.
A heuristic function is \emph{feasible} iff $w_{h_t} \geq 0$.
...
RPHAST, short for Restricted PHAST, is a PHAST extension for efficiently computing distances from a smaller set of source nodes to one target node.
Given a set of source nodes $S$ and a target node $t$, the first step is to copy all up-edges reachable only through up-edges from the source nodes into a new graph.
This step is called \emph{selection}.
Then, the PHAST algorithm is applied but the downward sweep (second step of the PHAST algorithm) is performed only on the new restricted subgraph.
...
% TODO not so great intro directly after PHAST/RPHAST
Route planning algorithms typically focus on the point-to-point shortest path problem.
...
In some cases on of the sets is a single node.
This is then called a \emph{one-to-many} or \emph{many-to-one} problem.
In this section, we study (discuss) a very specific setting arises naturally from A* heuristics.
...
We consider the combined running time of the target selection and each incremental query as the running time to answer such an incremental many-to-one query.
...
The core idea of our algorithm is to do the PHAST computation lazily and using memoization as depicted in Algorithm~\ref{algo:pot}.
...
\caption{Elimination Tree Lazy RPHAST Algorithm}
\label{algo:cch_pot}
\end{algorithm2e}

Algorithm~\ref{algo:pot} can be applied to CCH without any modifications.
However, similarly to the elimination tree query algorithm, we can also exploit the elimination tree for the Lazy RPHAST algorithm.
...
In~\cite{}, it was proven that the set of the ancestors in the elimination tree of a node $x$ equals the CH search space of $x$. % TODO cite
Since all upward neighbors of $x$ are by definition included in the search space, they are also ancestors in the elimination tree.
If the distance of the parent in the elimination tree has already been computed, the distances of all other upward neighbors have also been computed.
...
In this section, we discuss how the Lazy RPHAST schema can be applied in a Many-to-Many context.
...
Since our algorithm is a variation RPHAST, we can also apply it to the Many-to-Many problem.
% TODO explain
However, a naive approach would explore the upward search space of all the source over and over again.
This naive approach would not be competitive.
Luckily, we can do better by using a different One-to-Many CH variant -- the Bucket-CH algorithm -- to select all targets at once.
The Bucket CH selection phase works as follows:
For each node $x$, we maintain a list $B[x]$ of tuples $(t_i, dist(x,t_i))$, also called a \emph{bucket}.
These buckets can be populated by running a backward CH search from each $t_i$.
An even faster way to compute the buckets is to initialize a backward CH search with all nodes at once, order the queue by node rank and use the buckets as labels.
A bucket can be propagated along an edge by increasing each entry by the edge weight, and merged with another tentative bucket with a parallel sweep over both buckets.
% TODO way too dense
Once the buckets have been computed, we can compute distances to all targets once with Algorithm~\ref{algo:pot} (or~ Algorithm~\ref{algo:cch_pot}).
The only difference is that $D[x]$ is now an array of distances to each $t_i$.
We evaluate this algorithm in Section~\ref{}.
...
\subsection{CH-Potentials Heuristic}

CH-Pot is A* with Lazy RPHAST Pot.
Given $s$ and $t$ upward search from $t$
Then compute $h_t(x)$ for with ComputeAndMemoizeDist

\subsection{Bidirectional A*}

Bidir A* not as straightforward which one might expect.
As pointed out in~\cite{gh-cspas-05}, one can not simply use a forward potential $h_t$ for the forward search and a backward potential $h_s$ for the reverse search.
Since these searches would operate on different reduced graphs, it is unclear when the searches can terminate.
To the best of our knowledge, no better stopping criterion exists than running both searches until the minimum queue key $k$ becomes greater than the best known total tentative distance $\mu$.
That is the same stopping crit as for a unidir search..
symmetric
Thus, the symmetric bidirectional algorithm performs double the work than a simple unidir search.
can be slightly improved through pruning $\overrightarrow{D[x]} + h_t(x) > \mu$.
Still not competitive

average potential - reduced graph with average of reduced edge weights
but average pot in each dir worse than orig pot for dir
also overhead

we propose an improved pruning crit, one which takes the search progress into account (through the min queue keys), which makes the simpler symmetric approach competitive or even faster than the average pot
If $x$ was not yet settled by reverse search with minimum queue key $\overleftarrow{k}$ then
$\operatorname{dist}(x,t) \geq \overleftarrow{k} - h_s(x)$.
Thus, if $\overrightarrow{D[x]} + \overleftarrow{k} - h_s(x) > \mu$ we can discard $x$.

% direction switch strats!
