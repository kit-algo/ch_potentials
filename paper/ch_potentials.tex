...
Using the elimination tree, we obtain the algorithm in Algorithm~\ref{algo:cch_pot}.
For any vertex which already has a memoized distance $D[v] \neq \bot$, the algorithm immediately returns this distance.
Otherwise, the algorithm follows the elimination tree upward until a node with a finalized distance in encountered.
The elimination tree is represented as parent pointer array $P$.
The visited nodes are pushed onto a stack $S$.
This enables the algorithm to enumerate the vertices in revered order by popping them from the stack.
