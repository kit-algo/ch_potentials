...
Running two Dijkstra searches simultaneously, one from $s$ and a backward search from $t$, until the searches meet is called a \emph{Bidirectional Dijkstra}.\todo{Kann von mir aus bleiben, aber ich muss immer an Janus denken bei ``Bidirectional Dijkstra''.}
...
The searches can also be run in parallel in two threads.\todo{Bringt es uns etwas, dass hier Alternierungsstrategien aufgelistet werden, oder kann das weg?}
...
A* orders the nodes in the priority queue by $D[v] + h_t(v)$ instead of $D[v]$ as is done by Dijkstra's algorithm.
This results in nodes closer to the target being explored earlier.
Dijkstra's algorithm can be obtained as a special case of A* by setting $h_t(v)=0$.
...
For A* to be optimal, the heuristic needs to be feasible.
...
Customizable Contraction Hierarchies~\cite{dsw-cch-15} are a variant of Contraction Hierarchies that uses this three phase setup.
The key idea is to use a node importance order that is independent of the weight function.
As a CCH is also a CH, the same concepts such as the augmented graph $G^+$ exists.
...
Using a CH with a weight-specific importance order is often faster than a CCH with elimination tree query.
...
On road graphs, bidirectional search provides a way to halve the running time of Dijkstra's algorithm.\todo{sag niemals etwas sei ``easy''. Es gibt immer jemand der es schwer fand. Den schlägst du ins Gesicht.}
...
This setup provides an alternative way of integrating live traffic compared to the approach described in~\ref{sec:live-predicted-traffic}.
Changes to the live traffic situation are handled by changing $w_\ell$ via a customization.
Vehicle specific restrictions are accounted for in the query by adjusting $w_q$.
For severe traffic events, the three phase approach is likely faster.
...
\todo{Den Gedanken sollte man ausbauen oder streichen}
...
\todo{was ist die Alternierungsstrategie von Tabelle 3?}
