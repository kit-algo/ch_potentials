...
Routing applications often requires integrating the current traffic situation, planning ahead with traffic predictions for the future, respecting forbidden turns, and many other features depending on the exact application.
...
Both approaches achieve speed-ups of three orders of magnitude over Dijkstra's algorithm.
...
The submitted paper does not describe any of the contribution of \cite{kswz-erptd-p-20}.
...
We use 16 landmarks generate with the avoid strategy~\cite{gw-cppsp-05} and activate all during every query.
...
Our ALT implementation is uni-directional, as a bidirectional search creates problems for some settings, such as predicted traffic.
...
This heuristic has instant access to a shortest distance array with respect to $w_\ell$, i.e. it is the most efficient heuristic possible in our model.
