...
Our approach allows decoupling the supported applications from the hierarchical speedup technique.
...
The results on extending specific speedup techniques to specific extended problem fill many research papers~\cite{dgpw-crprn-13} and sometimes entire dissertations~\cite{b-tdrpc-14}. % TODO cite some papers
...
For example, while there are efficient speedup techniques for each of the above mentioned features, we are not aware of any work supporting the combination of these features.
We believe that for \emph{practical} algorithms, a different trade-off is necessary.
...
In the past two decades, a considerable amount of research effort has been put into accelerating shortest path computations in transportation networks.
...
A natural but rather ineffective heuristic can be obtained through euclidian distances.
On road networks, it performs worse than Dijkstra's algorithm~\cite{gh-cspas-05}.
...
This is because hierarchical techniques have proven much more effective.
...
This results in queries taking less than a tenth of a millisecond which is a speedup of four or more orders of magnitude over Dijkstra's algorithm.
The preprocessing takes only a few minutes.
...
These algorithms reuse the CH preprocessing but modify the query phase to efficiently support \emph{one-to-all} queries from one source vertex to \emph{all} other vertices (PHAST), or \emph{one-to-many} queries to a subset of vertices known in advance (RPHAST).
%
Another popular hierarchical speedup technique also used in practice~\cite{bingblog} is Multi-Level-Dijkstra~(MLD)~\cite{swz-umlgt-02} also known as CRP~\cite{dgpw-crprn-13}.
...
The speedup technique known to currently achieve the fastest queries is Hub Labels (HL)~\cite{TODO}.
With HL, query times in less than a microseconds are possible.
...
This approach is conceptually similar to ours.
...
Further, the evaluation was performed only with synthetic traffic data.
...
However, its flexibility is also limited.
Integrating traffic predictions into CRP was studied in~\cite{bdpw-dtdrp-16}.
Achieving reasonable memory consumption and query times was only possible by giving up exactness.
...
Even with all improvements proposed in~\cite{bwzz-cchtc-20}, turn integration still causes a slowdown of at least factor three to the customization phase.
...
Recently, a new approach has been published~\cite{swz-sfert-21} which simultaneously achieves competitive results in all three aspects but only at the cost of considerable implementation complexity. % TODO argue what is complicated to implement?
...
Clearly, there is a plethora of efficient speedup techniques for routing in road networks.
Further, for many extended problems there are dedicated research results on extensions of some techniques.
While many problems can be solved very efficiently, we still believe that this situation is not satisfactory in practice.
For practical applications, a simple, unified, and extensible approach is often more important than the fastest query performance.
Therefore, in this work, we revisit the A* algorithm and build a flexible, unified framework for a multitude of routing problems.
...
The augmented graph has the property that for any two vertices $s$ and $t$, there always exists an \emph{up-down} $st$-path, i.e.\ a path  which first uses only edges from $\gchu$ and then only edges from $\gchd$, with the same length as a shortest path in $G$.
...
Some hierarchical techniques were extended to support a three-phase setup~\cite{dgpw-crprn-13}.
...
Customizable Contraction Hierarchies~\cite{dsw-cch-15} (CCH) is a variant of Contraction Hierarchies that follows this three-phase setup.
...
The elimination tree query can therefore not be used with a classical CH.
...
\KwData{$P[v]$: parent of a vertex $v \in V$ in the elimination tree, a parent of $\bot$ indicates a root vertex}
...
To compute missing distances, the elimination tree ancestors which also do not have a distance yet are collected and pushed onto a stack $S$.
...
However, when the CH preprocessing and the A* algorithm are performed on the same graph with the same weight function, this yields a \emph{perfect} heuristic
...
\subsection{Formal Setup: Inputs, Outputs, and Phases}\label{sec:formal_framework}
...
In the applications considered in this paper, $w_\ell$ is always the \emph{freeflow} travel time.
...
The crucial requirement for a routing problem to fit into our framework is that the query weight of an edge $w_q(u v)$ is greater or equal to the shortest distance $\dist_\ell(\phi(u), \phi(v))$ between the corresponding vertices $\phi(u)$ and $\phi(v)$ in the lower bound graph $G_\ell$ with respect to $w_\ell$.
Typically, and unless stated otherwise, $G_q$ and $G_\ell$ are the same graph, $\phi$ is the identity function, and only $w_q$ changes for the queries.

\subsection{CH-Potentials}\label{sec:chpot}
CH-Potentials can be used to solve any problem in this framework.
The preprocessing is always the computation of the CH augmented graph and remains the same regardless of the specific routing problem.
...
The heuristic function $h_t(v)$ is implemented by a call to the \texttt{ComputeAndMemoizeDist} for vertex $v$ (see Algorithm~\ref{algo:pot}).
...
For this, we use CCH instead of CH preprocessing to build the augmented graph.
This allows us to introduce updates to $w_\ell$ and the augmented graph within a few seconds using the CCH customization algorithm.
...
In this table, we further include the sequential preprocessing running time to construct the augmented graphs and the space consumption.
...
In the simple $w_q = w_{\ell}$ setting, Hub Labels can be used to answer queries in less than a microsecond~\cite{TODO}, more than three orders of magnitude faster than with CH-Potentials.
Live traffic and arbitrary weight functions can be handled with CCH resulting in query times of around a 0.1\,ms~\cite{BuchholdSW19}.
CCH can also handle turn costs~\cite{bwzz-cchtc-20}.
With some algorithmic adjustments to the preprocessing, query times around an order of magnitude faster than CCH-Potentials are possible (0.3\,ms compared to 2.1\,ms with CCH-Potentials).
...
Based on Lazy RPHAST, we developed CH-Potentials, a fast, exact, and flexible two-phase technique based on A* and CH for finding shortest paths in road networks.
