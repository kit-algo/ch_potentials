...
% ich haette das at least eigentlich schon gerne drin. CH erreicht wesentlich mehr als 3 groessenordnungen
...
\centering
\caption{
CH and CH-Potentials performance for different route planning applications.
...
Table~\ref{tab:applications} mainly depicts the running times and CH-Potentials in the various applications, such as those described in Section~\ref{sec:extensions}.
For comparison, we also report CH running times.
A basic CH can only handle the $w_q = w_\ell$ case.
...
Another research avenue is investigating non-road graph types.
Much grid map research exists with series of competitions called GPPC \cite{DBLP:conf/socs/SturtevantTTUKS15}.
Hierarchical approaches work on these graphs \cite{DBLP:conf/aaai/UrasK14}.
