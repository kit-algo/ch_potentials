...
However, the algorithm development is vastly more complex. % TODO cite some papers
...
In~\cite{dsw-cch-15}, CH is extended to Customizable CH (CCH). % TODO paragraph looks ugly, somehow make more compact
...
After discussing notation and fundamental algorithms in Section~\ref{sec:preliminaries}, we introduce Lazy RPHAST, a simple and efficient CH-based algorithm for the incremental many-to-one shortest path problem in Section~\ref{sec:lazy-rphast}.
...
Our main contribution, the CH-Potentials framework is presented in Section~\ref{sec:framework} where we show how to use Lazy RPHAST to build a fast and tight heuristic for A* in road networks.
...
Problem extensions can be supported at the cost of some slowdown to the queries but without any algorithmic adjustments.
...
\emph{Contraction Hierarchies} (CH) is a two-phase speed-up technique to accelerate shortest path computations on road networks through precomputation.
