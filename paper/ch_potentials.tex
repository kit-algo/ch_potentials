...
With CH-Potentials, we thus get the tight heuristic $h$.
We set $h'(x,y) = h(y)$.
...
This is all that is necessary to combine live traffic and CH-Potentials.ic.
...
In our implementation, we to not modify the representation of $w_p$ and evaluate the formulas above at each heuristic evaluation.
...
We experimentally show that the running time of our algorithm is within a factor 1.6 of a hypothetical A* variant that can instantly evaluate the tight heuristic.
We conclude from this, that to achieve significantly smaller running times with A* based algorithms in road networks, variations of the problem setting are necessary.
