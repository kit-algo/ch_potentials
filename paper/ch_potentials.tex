...
A bidirectional variant of A* therefore appears also desirable.
However, as shown in~\cite{gh-cspas-05}, this is not as simple as it seems.

The straightforward idea would be to use two heuristics.
The forward search has its queue ordered by $\overrightarrow{D}[x] + \overrightarrow{h}_t(x)$ where $\overrightarrow{h}_t(x)$ estimates the distance from $x$ to $t$.
Similarly, the backward search has its queue ordered by $\overleftarrow{D}[x] + \overleftarrow{h}_s(x)$ where $\overleftarrow{h}_s(x)$ is an estimate of the distance from $s$ to $x$.
...
To obtain a better stopping criterion, the \emph{average potential} is proposed~\cite{gh-cspas-05}.
It combines a forward and a backward heuristic $\overrightarrow{h}_t$ and $\overleftarrow{h}_s$ into a combined heuristic $h_{\overline{st}}$, which is good for both directions.
Both searches run on the same reduced graph which enables an efficient stopping criterion.
The reduced graph of the average potential is the graph with the average weights of the reduced graphs of $\overrightarrow{h}_t$ and $\overleftarrow{h}_s$.
The heuristic function which induces this average reduced graph is $h_{\overline{st}}(x) := \frac{\overrightarrow{h}_t(x) - \overleftarrow{h}_s(x)}{2}$.
...
This makes it more expensive in terms of running time.
% Second, for each direction on its own, the average potential is a worse heuristic than the original heuristic.
Second, the better stopping criterion comes at the cost of worse estimates for each direction on its own.
The average potential estimates $\dist(x,t)$ less accurately than $\overrightarrow{h}_t(x)$ and $\dist(s,x)$ less accurately than $\overleftarrow{h}_s(x)$.
The second downside can be partially mitigated through pruning with the original heuristics.
When the forward search scans an edge $xy$ where $\overrightarrow{D}[x] + w(xy) + \overrightarrow{h}_s(y) > \mu$ holds, i.e. the distance plus the estimate of the remaining distance is already greater than the currently known tentative distance, $y$ needs not to be inserted into the queue.
The pruning for the backward search works analogously.
