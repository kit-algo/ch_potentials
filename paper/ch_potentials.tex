...
For every distance estimation, a shortest path to the target is computed, whose length is used as the heuristic value. % du hast mein "the" wieder rausgenommen, aber ohne Artikel klingt das fuer mich einfach falsch... Lasse mich aber gerne eines besseren belehren.
...
Preliminary experiments showed, that most A* running time is spent in heuristic evaluations and queue operations. % queue pushs stimmt hier nicht...
...
% glaub das mit dem ellispoid lenkt eher ab und braeuchte wenn dann noch eine ganze menge extraerklaerungen...
...
Achieving significantly smaller running times could still be possible in variations of the problem setting. % hab das mal ein bisschen mehr auf future work hin getrimmt
