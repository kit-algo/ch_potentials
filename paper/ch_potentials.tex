...
In this paper, we present Lazy RPHAST, an algorithm to efficiently compute distances from many nodes towards a single target and use it as an A* heuristic.
...
\Description[A map where nodes visited by A* are marked.]{
A map of Karlsruhe where nodes visited by A* are marked.
The shape of the area with the visited nodes is similar to a drop.
The source node roughly lies in the center of the drop, the target node at the tip.
}
\end{figure}
...
\label{fig:ch}
\Description[Nodes and edges of a shortest path in the original graph and the corresponding path in the augmented graph.]{
Nodes and edges of a shortest path in the original graph and the corresponding path in the augmented graph.
The y-position on the figure indicates the rank of each node.
Where the original shortest path down and then up, a shortcut edge connects the two higher nodes.
...
\begin{algorithm2e}
\KwData{$B[v]$: tentative distance from $v$ to target $t$}
\KwData{Minimum priority queue $Q$, also called open list, ordered by tentative distances}
$B[v] \gets +\infty$ for all $v\neq t$;
$B[t] \gets 0$\;
Make $Q$ only contain $t$ with weight $0$\;
\While{$Q$ not empty}{
  $u\gets$ pop minimum element from $Q$\;
  \For{all down-edges $vu$ in $G^+_\ell$}{
    \If{$B[v] > w_\ell(vu) + B[u]$}{
      $B[v]\gets w_\ell(vu) + B[u]$\;
                        Add $v$ or decrease $v$'s key in $Q$ to $B[v]$\;
    }
  }
}
...
Similarly, for a \emph{down path}, the first node has to be the mid node.
...
This is done by repeatedly contracting the least important node and adding shortcuts between its higher ranked neighbors.
...
Pseudocode for the backward search, i.e., the path from $m$ to $t$, is presented in Algorithm~\ref{algo:ch-backward}.
...
\begin{algorithm2e}
\KwData{$P[v]$: tentative distance from $v$ to $t$}
Execute Algorithm~\ref{algo:ch-backward}\;
\For{all CH levels $L$ from most to least important}{
	\For{all up-edges $uv$ in $G^+_\ell$ with $u$ in $L$}{
		\If{$P[u] < P[v] + w_\ell(uv)$}{
			$P[u] \leftarrow P[v] + w_\ell(uv)$\;
		}
	}
}
...
\begin{algorithm2e}
\KwData{$B[v]$: tentative distance from $v$ to $t$ as computed by Algorithm~\ref{algo:ch-backward}}
\KwData{$D[v]$: memoized distance from $v$ to $t$, $\bot$ initially}
\SetKwFunction{Dist}{ComputeAndMemoizeDist}
\SetKwProg{Fn}{Function}{:}{}
\Fn{\Dist{$u$}}{
	\If{$D[u] = \bot$}{
		$D[u]\leftarrow B[u]$\;
		\For{all up-edges $uv$ in $G^+_\ell$}{
                        $D[u]\leftarrow\min\{D[u],w_\ell(uv)+\Dist(v)\}$\;
		}
	}
	\Return{$D[u]$}\;
}
...
To compute the distance from a source $s_i$, we recursively compute for all up-edges $(s_i v)$ the distance $D[v]$.
Next, we compute the minimum distance over all up-down paths that contain at least one up-edge using $d = \min_v\{w_\ell(s_i v) + D[v]\}$.
...
\KwData{$B[v]$: tentative distance from $v$ to $t$ as computed by Algorithm~\ref{algo:ch-backward}}
\KwData{$D[v]$: memoized distance from $v$ to $t$, $\bot$ initially}
\KwData{$P[v]$: parent node of $v$ in the elimination tree, parent of root is $\bot$}
...
\Fn{\Dist{$u$}}{
  ...
  \While{$S$ not empty}{
    $v\leftarrow$ pop top element from $S$\;
		$D[v]\gets B[v]$\;
		\For{all up-edges $vw$ in $G^+_\ell$}{
                        $D[v]\gets\min\{D[v],w_\ell(vw)+D[w]\}$\;
		}
	}
	\Return{$D[u]$}\;
}
...
The heuristic function $h_t(v)$ is implemented by a call to the \texttt{ComputeAndMemoizeDist} for node $v$.
...
Our algorithm uses the undirected degree $d(v)$ of a node $v$.
Formally, $d(v)$ is the number of nodes $u$ such that $(vu)\in E$ or $(uv)\in E$.

Analogous to A*, our algorithm stores for every node $v$ a tentative distance $D[v]$.
Additionally, it maintains a minimum priority queue.
Diverging from A*, not all nodes need be pushed, but every node has a tentative distance.

\subsubsection{Skip Degree Two Nodes}

Our algorithm differs from A* when removing a node $u$ from the queue.
...
Our algorithm, however, behaves differently, if $d(v)\le 2$.
Our algorithm determines the longest degree two chain of nodes $u, v_1,\ldots, v_k, w$ such that $d(v_i)=2$ and $d(w) > 2$.
...
If $D[w]$ is modified and $d(w)>2$, $w$'s weight in the queue is set to $D[w]+h_t(w)$.
...
If $d(w) > 3$ or $w$ is in the queue, our algorithm proceeds as in the previous section.
...
When the forward search scans an edge $uv$ where $\overrightarrow{D}[u] + w(uv) + \overrightarrow{h}_s(v) > \mu$ holds, i.e. the distance plus the estimate of the remaining distance is already greater than the currently known tentative distance, it is not necessary to push $y$ into the queue.
...
Nodes are removed from the backward queue ordered by $\overleftarrow{h}(v) + \dist(v,t)$.
...
The node $v$ might still be pushed into the queue when there is another edge $wv$ for which the pruning is not possible.
...
The A* search uses the tentative distance $\tau$ at a node $u$ when evaluating the travel time of outgoing edges $u v$.
...
The x-axis in both graphs is the number of nodes that were queried up to that point in the incremental query.
...
While the numbers are not perfectly comparable due to different benchmark machines, we can still conclude that Lazy RPHAST is a very useful extension to RPHAST.
...
The first heuristic is the \emph{Zero} heuristic where $h(v)$ is $0$ for all nodes $v$.
...
This shows that CH-Potentials gracefully converge toward CH in the $w_q = w_\ell$ special case.
