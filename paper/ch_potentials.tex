...
\pdfinfo{
/Title (A* with Fast and Tight Heuristic/Potentials in Road Networks)
...
\title{A* with Fast and Tight Heuristic/Potentials in Road Networks}
\author{Ben Strasser\\
...
Computing a good heuristic function is a challenge of its own.
To avoid the need for a heuristic function, many hierarchical speedup techniques, such as Contraction Hierarchies (CH), have been developped.
These techniques are not based on A* and therefore do not need a heuristic function.
They achieve speed and optimality but sacrifize A*'s flexibility.
In this paper, we use hierarchical techniques to compute heuristic functions.
...
Following, their terminology, we refer to the heuristic as \emph{potential} function $\pi$.
Further, paths are computed from a source node $s$ to a target node $t$.

We say that a potential $\pi$ is \emph{tight}, if no tighter potential can be computed in the preprocessing phase.
The shortest path distances with respect to $w_\ell$ are tight.
CH-Potentials compute $w_\ell$ shortest paths and are thus tight.
...
However, ALT's potentials are not tight. 
%
...
The resulting potential is tight.
...
However, is a lower bound for every algorithm following our setup.
...
The potentials are calculated utilizing a CH on a lower bound graph and tight with regards to that lower bound graph.
