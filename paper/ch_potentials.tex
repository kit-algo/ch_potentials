...
Road intersections become nodes.
Road segments become edges.
The weight of an edge is the time a car needs to traverse the road segment.
The problem can be solved with Dijkstra's algorithm~\cite{d-ntpcg-59}.
However, on continental sized networks, it is too slow for many applications.
Thus, speed-up techniques have been developed.
A popular example for such a technique, which has been successfully used in many real world applications, is Contraction Hierarchies~(CH)~\cite{gssv-erlrn-12}.
CH exploit the inherent hierarchy of road networks.
In a preprocessing step, additional shortcut edges are inserted, which allow skipping over unimportant parts of the network during query time.
Another technique, also successfully employed in practice, is Multi-Level-Dijkstra~(MLD)~\cite{dgpw-crprn-13} better known as CRP.
It also exploits the hierarchy of road networks using shortcut edges.
...
However, for many real world routing applications, this basic model turns out to be too simple.
For realistic routing, many additional features need to be considered.
This includes but is not limited to turn costs and restrictions, live traffic, user preferences, traffic predictions, temporary driving bans for certain types of vehicles and many more.
Hierarchical speed-up techniques can be extended to support these features.
However, each feature requires significant effort.
Many combinations of specific techniques with certain extensions have their own dedicated research paper(s).
Adding these features introduces significant additional implementation complexity.
Supporting combinations of extensions is at least non-trivial, oftentimes downright impossible.

In this work, we propose a novel building block for routing algorithms which drastically improves on this extensibility problem with very little implementation complexity.
For this, we revisit goal-directed search (A*).
Goal directed search is a simple extension of Dijkstra's algorithm.
An estimate of the remaining distance to the target is used, to change the order in which nodes are traversed during the search.
This guides the search towards the target.
See Figure~\ref{img:search-space} for an example of nodes traversed during a goal-directed search.
This approach is far easier to extend.
Many extensions can be supported without modifying the distance estimation at all.
The downside is that the effectiveness of this approach highly depends on the tightness of the estimate.
There has been extensive research on goal-directed search in the past~\cite{}.
However, on road networks, previous approaches could not deliver performance on par with hierarchical techniques.
We introduce CH-Potentials, a new distance estimation function which can efficiently compute tight distance estimates.
\todo{tight}
CH-Potentials are a simple combination of existing approaches.
CH-Potentials use hierarchical speed-up techniques but only indirectly through goal directed search.
This new building block allows handling extensions orthogonal to the speed-up technique.
\todo{contribution - no better heuristic possible}
% By comparing against Oracle-A*, we show that no other exact two-phase A*-based approach can be significantly faster.
In addition, we propose optimizations for Dijkstra's/A*'s handling of low-degree nodes, common in road networks.
