...
% TODO Capital letters for arrays?
...
% TODO so far, we only have levels but not ranks
...
A bidirectional variant of A* therefore is therefore also desirable.
However, as shown in~\cite{gh-cspas-05}, the necessary modifications are not straight forward.
...
% We denote by $\overrightarrow{D}[x]$ the tentative distance of the forward search at $x$ and similarly, by $\overleftarrow{D}[x]$ the tentative distance in the backward search.
...
Thus, this straightforward bidirectional A* performs roughly twice the work of a unidirectional A*.
This is not a useful algorithm.
However, it can serve as basis for further algorithmic refinements.
The authors of~\cite{gh-cspas-05} therefore refer to this approach as \emph{symmetric} bidirectional A*.
...
The forward search uses $h_{\overline{st}}(x)$ as heuristic.
The backward search uses $-h_{\overline{st}}(x)$.
This results in both searches running on the same reduced graph.
The weights of the combined reduced graph corresponds to the averaged weights of the individual reduced graphs.\todo{Ist diese Aussage korrekt?}
This allows stopping the searches when $\overrightarrow{k} + \overleftarrow{k} \geq \mu$.
To the best of our knowledge, using this stopping criterion yields the fastest known bidirectional A* variant.
...
Evaluating the average heuristic is therefore slower than evaluating just one the composing heuristics.
...
When the forward search scans an edge $xy$ where $\overrightarrow{D}[x] + w(x,y) + \overrightarrow{h}_s(y) > \mu$ holds, i.e. the distance plus the estimate of the remaining distance is already greater than the currently known tentative distance, $y$ needs not to be inserted into the queue.\todo{does not have to be inserted? needs not klingt komisch... aber English ist nicht meine Stärke. :) }
...
We do not use the average heuristic and use the unidirectional stopping criteria.
...
Before pushing $y$ into the queue, we apply the pruning rule.
...
We therefore want to obtain a lower bound for $\dist(s,x)+w(x,y)+\dist(y,t)$.
As $x$ was settled, $\overrightarrow{D}[x]$ contains the shortest path length $\dist(s,x)$, i.e., $\dist(s,x)=\overrightarrow{D}[x]$.
$w(x,y)$ is also exactly known as it's just an edge weight.
It remains to lower bound $\dist(y,t)$.
Nodes are removed from the backward queue ordered by $\overrightarrow{h}(y) + \dist(y,t)$.
\todo{$\ge$ vs $\geq$ und $\le$ vs $\leq$, macht das einen Unterschied?}
If $y$ was not yet removed from the backward queue, we know that $\overrightarrow{h}(y) + \dist(y,t) \geq \overleftarrow{k}$.
This gives us the required lower bound $\dist(y,t) \geq \overleftarrow{k}-\overrightarrow{h}(y)$.
$y$ does not have to be pushed, if it was not settled by the backward search and $\overrightarrow{D}[x]+w(x,y)+\overleftarrow{k}-\overrightarrow{h}(y)\ge \mu$.
This does not mean that $y$ will never be pushed into the forward queue.
It is possible that another edge $zy$ exists for which the pruning rule cannot be applied.

\todo{Latex mag overrightarrow nicht in todos... Ich packe die Frage deswegen inline in den Text.}
Kann man sinnvoll prunen wenn $y$ von der Rückwärtssuche gesettled wurde? Z.b. Kante $xy$ prunen wenn $x$ forward gestelled ist und $y$ backward gesettled ist und $\overrightarrow{D}[x]+w(x,y)+\overleftarrow{D}[y]\ge \mu$ erfüllt ist.

\todo{Der folgende Paragraph ist die Vorgängerversion vom Paragraph davor. Wenn der neue Paragraph besser ist, dann kann dieser weg.}
We use the minimum queue key $\overleftarrow{k}$ of the backward search to obtain a tighter estimate of $\dist(x,t)$ for pruning in the forward search.
Recall that queues are ordered by $D[x] + h(x)$.
If a node $x$ was not yet settled by backward search then $\dist(x,t) + \overleftarrow{h}_s(x) \geq \overleftarrow{k}$, and equivalently $\dist(x,t) \geq \overleftarrow{k} - \overleftarrow{h}_s(x)$.
...
As the pruning becomes tighter as the searches progress, it compensates for the worse stopping criterion.
...
To evaluate Lazy RPHAST in the incremental setting, we measure the elapsed running time after $2^i$ result distances were computed. % TODO word smithing "result distances"
% TODO selection not included
