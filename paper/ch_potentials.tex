...
The searches can terminate when the sum of the minimum keys in both queues $\overrightarrow{k} + \overleftarrow{k}$ is greater than the so far best known tentative total distance $\mu$.
...
% TODO formatting
After calling the adjusted ComputeAndMemoizeDist for each source, we know the shortest distances between each pair of source and target node.
...
Then, the heuristic function $h_t(x)$ is implemented by call the ComputeAndMemoizeDist for node $x$.
...
Important feature alternative routes
One popular framework is the penalty method
It computes an alternative graph instead of a set of routes
the graph is constructed iteratively
in each iteration, first, a shortest path between s and t is computed
if the path or certain parts of it meet certain quality criteria add to alternative graph
finally penalize all edge weights of current path and possibly adjoined edges and continue until a certain stopping criterion is fulfilled

running time dominated by shortest path computations
since edge weights only increased natural fit for A* with CH-Pot
lots of details to implement and define
we take config of kobitzsch diss
quality eval over there
here only performance

for sake of reproducibility we briefly discuss the details of our configuration
code from authors
eps/psi = 1.25
penalization schema
	exp psi
