...
In this work, we revisit the A* algorithm to build a simple, extensible and unified algorithmic framework applicable to many route planning problems. % without adjustments to the preprocessing.
...
For every other vertex, its parent is its least important upward neighbour.
...
However, on weight functions with a less pronounced hierarchy, CH performance degrades significantly, and CCH queries turn out to be the faster variant.
...
Such a level assignment can be obtained by assigning all vertices without lower-ranked neighbours to the lowest level.
All other vertices are then iteratively assigned to the next level above the highest level of any downward neighbour.
...
This section discusses a variant of the many-to-one shortest path problem, which naturally arises from A* heuristics.
...
The core idea of our algorithm is to do the RPHAST computation lazily using memoization.
...
Further, the distances $D[v]$ are initialized to a sentinel value $\bot$ where $\bot$ is some particular value distinct from any other valid distance (including $\infty$).
...
\Fn{\Dist{$u$}}{
  ...
  \While{$S$ not empty}{
    $v\leftarrow$ pop top element from $S$\;
		...
		\For{all up-edges $vx \in \echu$}{
      $D[v]\gets\min(D[v],w^+(vx)+D[x])$\;
		}
	}
	\Return{$D[u]$}\;
}
...
Otherwise, the algorithm follows the elimination tree upward until a node with a finalized distance is encountered.
The elimination tree is represented as parent pointer array $P$.
The visited nodes are pushed onto a stack $S$.
This enables the algorithm to enumerate the vertices in reversed order by popping them from the stack.
While popping the nodes, all outgoing upward edges are relaxed.
...
\Description{
A double logarithmic line plot with six lines with similar shapes and absolute values.
Running times for both Lazy RPHAST variants on both graphs graphs with a ball size of 16384 is around 1ms.
Classical RPHAST takes up to 2ms.
For a ball size of 2 to the 20 CH-based Lazy RPHAST variants need 10ms and CCH based variants slightly more.
For a ball size of 2 to the 24 CH-based Lazy RPHAST variants are at 30ms and CCH based variants slightly above 40ms.
}
\end{figure}
...
% On the one hand, Lazy RPHAST needs to do slightly less work and looks at each edge in the CH search space only once.
% On the other, RPHAST looks at edges in the search space both during the selection and the query but has a more pipeline and cache-friendly organization.
...
% We can, therefore, even expect competitive performance compared to other many-to-one algorithms.
We conclude that Lazy RPHAST is a very valuable extension of RPHAST.
% It is the first algorithm to efficiently support accelerated shortest path computations with dynamic source sets.
% Further, it even features competitive running times in the setting where both the target and the source set change between queries.
...
% This enables CCH-Potentials to support a customization phase, whereas CH-Potentials cannot.
