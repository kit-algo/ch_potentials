...
Handling complex weights, such as edge weight functions, is another aspect of flexibility.
...
We call our algorithm \emph{CH-Potentials}, following the ALT terminology.
...
However, a lot of past work uses A* to accelerate hierarchical approaches even further and loose A*'s' flexibility.
...
For every distance estimation, a shortest path to the target is computed, whose length is used as tight heuristic.
...
This results in a good but not a tight heuristic.
%
...
By comparing against Oracle-A*, we show that no two-phase alternative exact A* approach can be significantly faster.
...
Using a CH, we can compute a simple and tight heuristic.
...
Preliminary experiments showed, that most A* running time is spent in heuristic evaluations and queue push operations.
As both appear in pairs, reducing the number of push operations, is beneficial for both.
Avoiding push operations on low degree nodes is the focus of this section.
...
In~\cite{kswz-erptd-20}, a preliminary version of CH-Potentials is used is such scenario.
...
To obtain the routing graph, we use the import from RoutingKit\footnote{\url{github.com/RoutingKit/RoutingKit}}.
...
Additionally, we have two graphs with proprietary traffic predictions provided by PTV\footnote{\url{ptvgroup.com}}.
...
The search space of A* is an ellipsoid.
The number explored A* nodes depends on the density of the area covered by this shape.
As the density varies, the running times vary.

Table~\ref{tab:building_blocks} shows the impact of our optimizations.
We run queries with three heuristic.
...
Oracle-A* is unbeatable theoretic lower bound.
The reported running time of Oracle-A* do \emph{not} account for the heuristic evaluation.
We include Oracle-A* as it is a lower bound not only for our algorithm but every related work.
...
Our algorithm is only 1.6 times slower than an unachievable theoretic lower bound.
This means that no competitor algorithm such as ALT or CPD-Heuristics can be significantly faster than CH-Potentials.
...
This is the only problem variant solved by the basic CH algorithm.
...
In the other scenarios, the performance of CH-Potentials strongly depends on the quality of the tight heuristic.
...
This increase is mostly due to the BCC optimization becoming ineffective when considering turns.
...
The approach can handle a multitude of routing scenarios with very little implementation complexity.
The performance of CH-Potentials crucially depends on the availability of good weight lower bounds in preprocessing phase.
Our experiments show, that this availability is highly application dependent.
We experimentally show that the running time of our algorithm is within a factor 1.6 of a hypothetical A* variant that can instantly evaluate the heuristic.
We conclude from this, that to achieve smaller running times with A* based algorithms variations of the problem setting are necessary.
