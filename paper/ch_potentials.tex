...
A* iterates over the outgoing edges $(xy)$ of $x$ and tries to reduce $D[y]$ by relaxing $(xy)$.
...
When the forward search scans an edge $xy$ where $\overrightarrow{D}[x] + w(xy) + \overrightarrow{h}_s(y) > \mu$ holds, i.e. the distance plus the estimate of the remaining distance is already greater than the currently known tentative distance, it is not necessary to push $y$ into the queue.
