...
// This is the standard compilant path
#define aligned_free(ptr) free(ptr)

#else

// Not all compilers support aligned_alloc such as GCC 4.6.
// GCC 4.8 also needs these if you compile with ASAN.
// GCC-4.8 ASAN complaints when calling free on a pointer returned by aliged_alloc even though that is ok.

void*my_aligned_alloc(uint8_t alignment, uint64_t size){
	assert(alignment != 0);
...
#define aligned_alloc(alignment, size) my_aligned_alloc(alignment, size)
...
namespace {
	#ifndef ROUTING_KIT2_NO_GCC_EXTENSIONS
